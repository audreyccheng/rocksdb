...
class Transaction {
 public:
  ...
  virtual Status DeleteKey(const Slice& key) = 0;
  ...
  virtual const std::map<std::string, std::string> & GetWriteValues() const { return write_values_; }

  virtual void ClearReadVersions() { read_versions_.clear(); }
